...
int CsvRecord::load(FileReader *r)
{
    int h=1;
    CsvField *fld;
    while(h==1)
    {
        fld=new CsvField(top->settings);
        h=fld->load(r);
        addField(fld);
    }
    if(h==0)return 1;
...
{
    CsvField *fld=new CsvField(top->settings);
    addField(fld);
}
...
void CsvRecord::print(FILE *f)
{
    ...
    {
        record[i]->print(f);
        if(i+1!=record_size)fprintf(f,"%c",top->settings->field_seperator_out);
    }
}
void CsvRecord::print(FILE *f,int *col)
{
    ...
    for(i=0; i<ln; i++)
    {
        if(i==col[p])
        {
            ...
            p++;
            if(i+1<ln && col[p]!=INT_MAX)fprintf(f,"%c",top->settings->field_seperator_out);
...
{
    print(f);
    fprintf(f,"%c",top->settings->record_seperator_out);
}
